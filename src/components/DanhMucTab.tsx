  ...
  return (
    <div className="p-6 space-y-6 font-sans bg-gray-50 dark:bg-slate-900/50/50 min-h-[calc(100vh-4rem)]">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-gray-900 dark:text-white tracking-tight">Danh Mục Cơ Sở</h1>
          <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Khai báo cấu trúc danh sách sản phẩm, hồ sơ đối tác và hệ thống quỹ thu chi</p>
        </div>
      ...
      <div className="flex flex-wrap border-b border-gray-200 dark:border-slate-800">
        <button
          ...
          className={`flex items-center gap-1.5 px-5 py-3 border-b-2 text-xs font-bold transition whitespace-nowrap ${activeSubTab === 'products' ? 'border-[#0a8251] text-[#0a8251]' : 'border-transparent text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:text-white'}`}
        >
        ...
        <button
          ...
          className={`flex items-center gap-1.5 px-5 py-3 border-b-2 text-xs font-bold transition whitespace-nowrap ${activeSubTab === 'customers' ? 'border-[#0a8251] text-[#0a8251]' : 'border-transparent text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:text-white'}`}
        >
        ...
        <button
          ...
          className={`flex items-center gap-1.5 px-5 py-3 border-b-2 text-xs font-bold transition whitespace-nowrap ${activeSubTab === 'suppliers' ? 'border-[#0a8251] text-[#0a8251]' : 'border-transparent text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:text-white'}`}
        >
        ...
        <button
          ...
          className={`flex items-center gap-1.5 px-5 py-3 border-b-2 text-xs font-bold transition whitespace-nowrap ${activeSubTab === 'funds' ? 'border-[#0a8251] text-[#0a8251]' : 'border-transparent text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:text-white'}`}
        >
      ...
      <div className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-gray-100 dark:border-slate-800/50 shadow-xs flex items-center">
        ...
        {activeSubTab === 'products' && (
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-gray-50 dark:bg-slate-900/50 text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase border-b border-gray-100 dark:border-slate-800/50">
              ...
              <tbody className="divide-y divide-gray-100 text-xs">
                {products.filter(p => p.sku.toLowerCase().includes(searchQuery.toLowerCase()) || p.name.toLowerCase().includes(searchQuery.toLowerCase())).map((p) => {
                  ...
                  return (
                    <tr key={p.id} className="hover:bg-gray-50 dark:bg-slate-900/50/50">
                      <td className="py-3 px-4 font-mono font-bold text-gray-700 dark:text-gray-300">{p.sku}</td>
                      <td className="py-3 px-4 font-semibold text-gray-900 dark:text-white">{p.name}</td>
                      <td className="py-3 px-4 text-center text-gray-500 dark:text-gray-400 font-medium">{p.unit}</td>
                      <td className="py-3 px-4 text-right text-gray-900 dark:text-white font-bold">{formatVND(p.inputPrice)}</td>
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-gray-100 text-xs">
                {(activeSubTab === 'customers' ? customers : suppliers)
                  ...
                  .map((c) => (
                    <tr key={c.id} className="hover:bg-gray-50 dark:bg-slate-900/50/50">
                      <td className="py-3 px-4 font-semibold text-gray-900 dark:text-white">{c.name}</td>
                      <td className="py-3 px-4 font-mono text-gray-600 dark:text-gray-400 font-medium">{c.phone || 'Chưa cập nhật'}</td>
                      <td className="py-3 px-4 text-gray-500 dark:text-gray-400">{c.address || 'Chưa cập nhật'}</td>
                      <td className="py-3 px-4 font-mono font-bold text-gray-700 dark:text-gray-300">{c.mst || '--'}</td>
                      <td className="py-3 px-4 text-center">
                        <button
                          onClick={() => {
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-gray-100 text-xs">
                {fundAccounts.filter(fa => fa.name.toLowerCase().includes(searchQuery.toLowerCase())).map((fa) => (
                  <tr key={fa.id} className="hover:bg-gray-50 dark:bg-slate-900/50/50">
                    <td className="py-3 px-4 font-semibold text-gray-900 dark:text-white">{fa.name}</td>
                    ...
                    <td className="py-3 px-4 font-mono text-gray-600 dark:text-gray-400 font-bold">{fa.accountNumber || 'Không có (Tiền mặt)'}</td>
                    <td className="py-3 px-4 text-right text-gray-900 dark:text-white font-extrabold">{formatVND(fa.initialBalance)}</td>
                    <td className="py-3 px-4 text-center">
                      <button
                        onClick={() => {
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-xs flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-slate-900 rounded-xl shadow-2xl max-w-md w-full overflow-hidden animate-in fade-in zoom-in-95 duration-150">
            <div className="p-4 bg-[#0a8251] text-white flex items-center justify-between">
              ...
              <button onClick={() => setShowAddModal(false)} className="p-1 hover:bg-white dark:bg-slate-900/20 rounded text-white transition">
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <form onSubmit={handleSubmit} className="p-5 space-y-4 text-xs">
              ...
              {activeSubTab === 'products' && (
                <>
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="block text-[11px] font-bold text-gray-500 dark:text-gray-400 mb-1">Mã SKU *</label>
                      <input
                        type="text"
                        required
                        value={prodSku}
                        onChange={(e) => setProdSku(e.target.value)}
                        className="w-full bg-gray-50 dark:bg-slate-900/50 border border-gray-200 dark:border-slate-800 rounded-lg px-3 py-2 text-xs focus:bg-white dark:bg-slate-900 focus:ring-1 focus:ring-[#0a8251] outline-none font-mono"
                      />
                    </div>
                    <div>
                      <label className="block text-[11px] font-bold text-gray-500 dark:text-gray-400 mb-1">Đơn vị tính *</label>
                      <input
                        type="text"
                        required
                        value={prodUnit}
                  ...
                  <div>
                    <label className="block text-[11px] font-bold text-gray-500 dark:text-gray-400 mb-1">Tên vật tư / sản phẩm *</label>
                    <input
                      type="text"
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="block text-[11px] font-bold text-gray-500 dark:text-gray-400 mb-1">Đơn giá mua (giá vốn) *</label>
                      <input
                        type="number"
                        placeholder="đ"
                        required
                        ...
                        className="w-full bg-gray-50 dark:bg-slate-900/50 border border-gray-200 dark:border-slate-800 rounded-lg px-3 py-2 text-xs focus:bg-white dark:bg-slate-900 focus:ring-1 focus:ring-[#0a8251] outline-none text-right font-semibold"
                      />
                    </div>
                    <div>
                      <label className="block text-[11px] font-bold text-gray-500 dark:text-gray-400 mb-1">Đơn giá bán sỉ/lẻ *</label>
                      <input
                        type="number"
                        placeholder="đ"
                        required
                        ...
                        className="w-full bg-gray-50 dark:bg-slate-900/50 border border-gray-200 dark:border-slate-800 rounded-lg px-3 py-2 text-xs focus:bg-white dark:bg-slate-900 focus:ring-1 focus:ring-[#0a8251] outline-none text-right font-semibold text-[#0a8251]"
                      />
                    </div>
                  </div>

                  <div>
                    <label className="block text-[11px] font-bold text-gray-500 dark:text-gray-400 mb-1">Nhóm biểu phí thuế (Thông tư 40) *</label>
                    <select
              ...
              {(activeSubTab === 'customers' || activeSubTab === 'suppliers') && (
                <>
                  <div>
                    <label className="block text-[11px] font-bold text-gray-500 dark:text-gray-400 mb-1">Tên đối tác / Tên Đơn vị *</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-[11px] font-bold text-gray-500 dark:text-gray-400 mb-1">Số điện thoại</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-[11px] font-bold text-gray-500 dark:text-gray-400 mb-1">Địa chỉ liên lạc giao dịch</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-[11px] font-bold text-gray-500 dark:text-gray-400 mb-1">Mã số thuế doanh nghiệp (nếu có)</label>
                    <input
                      type="text"
                      placeholder="MST..."
                      value={contactMst}
              ...
              {activeSubTab === 'funds' && (
                <>
                  <div>
                    <label className="block text-[11px] font-bold text-gray-500 dark:text-gray-400 mb-1">Tên tài khoản quỹ *</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-[11px] font-bold text-gray-500 dark:text-gray-400 mb-1">Phân loại quỹ *</label>
                    <select
                      value={fundType}
                  ...
                  {fundType === 'bank' && (
                    <div>
                      <label className="block text-[11px] font-bold text-gray-500 dark:text-gray-400 mb-1">Số tài khoản ngân hàng *</label>
                      <input
                        type="text"
                        ...
                        className="w-full bg-gray-50 dark:bg-slate-900/50 border border-gray-200 dark:border-slate-800 rounded-lg px-3 py-2 text-xs focus:bg-white dark:bg-slate-900 focus:ring-1 focus:ring-[#0a8251] outline-none font-mono font-bold"
                      />
                    </div>
                  )}

                  <div>
                    <label className="block text-[11px] font-bold text-gray-500 dark:text-gray-400 mb-1">Số dư khởi đầu kỳ hạch toán (đ) *</label>
                    <input
                      type="number"
                      ...
                      className="w-full bg-gray-50 dark:bg-slate-900/50 border border-gray-200 dark:border-slate-800 rounded-lg px-3 py-2 text-xs focus:bg-white dark:bg-slate-900 focus:ring-1 focus:ring-[#0a8251] outline-none text-right font-extrabold"
                    />
                  </div>
                </>
              )}
