    ...
    <div className="p-6 space-y-6 font-sans bg-gray-50/50 min-h-[calc(100vh-4rem)]">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-0.5">Thống kê kinh doanh của bạn</p>
        </div>
        ...
        <div className="flex items-center gap-2 bg-white px-3 py-2 rounded-xl border border-gray-200 shadow-xs">
          ...
          <input
            ...
            className="text-xs text-gray-700 bg-transparent border-none focus:ring-0 cursor-pointer"
          />
      ...
      <div className="border-b border-gray-200 flex items-center justify-between">
        <div className="flex gap-6">
          ...
          <button className="pb-3 text-sm font-medium text-gray-500 hover:text-gray-900 transition flex items-center gap-1.5">
            <span>Tin tức</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-5 space-y-6">
          ...
          <div className="bg-white rounded-xl border border-gray-100 shadow-xs p-5 flex flex-col items-center text-center">
            ...
            <h2 className="text-lg font-bold text-gray-900 mt-2 flex items-center gap-1.5">
          ...
          <div className="bg-white rounded-xl border border-gray-100 shadow-xs p-5">
            <div className="flex items-center justify-between mb-4">
              <div>
                <h3 className="text-sm font-bold text-gray-800">Theo doanh thu</h3>
              ...
              <div className="flex items-center gap-1.5">
                <button 
                  ...
                  className="p-1 rounded bg-gray-50 hover:bg-gray-100 disabled:opacity-40 transition"
                >
                  <ChevronLeft className="w-4 h-4 text-gray-500" />
                </button>
                <span className="text-[11px] text-gray-500 font-medium">{revenuePage}/2</span>
                <button 
            ...
            {revenuePage === 1 ? (
              <div className="space-y-3">
                <div className="flex justify-between items-center py-2 border-b border-gray-50 text-xs">
                  <span className="text-gray-500 font-medium">Doanh thu bán hàng</span>
                  <span className="text-gray-900 font-bold">{formatVND(grossSalesRevenue)}</span>
                </div>
                <div className="flex justify-between items-center py-2 border-b border-gray-50 text-xs">
                  <span className="text-gray-500 font-medium">Các khoản giảm trừ</span>
                  <span className="text-gray-900 font-bold text-red-500">-{formatVND(totalDiscounts)}</span>
                </div>
                <div className="flex justify-between items-center py-2 border-b border-gray-50 text-xs bg-gray-50/50 px-2 rounded">
                  <span className="text-gray-700 font-bold">Doanh thu thuần</span>
                  <span className="text-[#0a8251] font-extrabold">{formatVND(netRevenue)}</span>
                </div>
                <div className="flex justify-between items-center py-2 border-b border-gray-50 text-xs">
                  <span className="text-gray-500 font-medium">Giá vốn hàng bán</span>
                  <span className="text-gray-900 font-bold">{formatVND(costOfGoodsSold)}</span>
                </div>
                <div className="flex justify-between items-center py-2 border-b border-gray-50 text-xs bg-[#eaf4f0]/30 px-2 rounded">
                  <span className="text-gray-700 font-bold">Lợi nhuận gộp</span>
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center py-2 border-b border-gray-50 text-xs">
                  <span className="text-gray-500 font-medium">Chi phí quản lý phát sinh</span>
                  <span className="text-gray-900 font-bold text-amber-600">-{formatVND(operatingExpenses)}</span>
                </div>
                <div className="flex justify-between items-center py-2 border-b border-gray-50 text-xs bg-[#eaf4f0]/40 px-2 rounded">
                  <span className="text-gray-800 font-extrabold">Lợi nhuận trước thuế</span>
                  <span className="text-emerald-700 font-extrabold">{formatVND(netProfitBeforeTax)}</span>
                </div>
                <div className="flex justify-between items-center py-2 border-b border-gray-50 text-xs">
                  <span className="text-gray-500 font-medium">Thuế GTGT đầu ra ước tính</span>
                  <span className="text-gray-900 font-bold">{formatVND(totalVat)}</span>
                </div>
        ...
        <div className="lg:col-span-7 space-y-6">
          ...
          <div className="bg-[#eaf4f0] rounded-xl p-5 border border-green-100 flex gap-4 items-start shadow-xs">
            <div className="w-10 h-10 rounded-full bg-white text-[#0a8251] flex items-center justify-center shrink-0 shadow-xs">
              <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364-6.364l-.707.707M6.343 17.657l-.707.707m0-12.728l.707.707m12.728 12.728l.707.707M12 8a4 4 0 100 8 4 4 0 000-8z" />
              </svg>
            </div>
            <div>
              <h3 className="font-bold text-sm text-gray-800">Chào {businessProfile.representative},</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="bg-white rounded-xl border border-gray-100 shadow-xs p-5 flex flex-col justify-between min-h-[220px]">
              <div>
                <div className="flex items-center justify-between mb-4">
                  <span className="text-xs font-bold text-gray-800">Số quỹ</span>
                  <div className="flex items-center gap-1.5">
                    <button 
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-xl font-extrabold text-gray-900">{formatVND(activeFund.balance)}</p>
                </div>
              </div>
            ...
            <div className="bg-white rounded-xl border border-gray-100 shadow-xs p-5 flex flex-col min-h-[220px]">
              <span className="text-xs font-bold text-gray-800 mb-3 block">Danh sách tài khoản ngân hàng</span>
              <div className="space-y-3 flex-1 overflow-y-auto max-h-[160px] pr-1">
                {calculatedFundBalances.filter(acc => acc.type === 'bank').map((acc) => (
                  <div key={acc.id} className="p-3 rounded-lg bg-gray-50/50 border border-gray-100 flex items-center justify-between gap-2">
                    <div>
          ...
          <div className="bg-white rounded-xl border border-gray-100 shadow-xs p-5">
            <h3 className="text-xs font-bold text-gray-800 mb-3 uppercase tracking-wider">Chi phí vận hành theo khoản mục</h3>
            <div className="space-y-3">
              {filteredTxs.filter(tx => tx.type === 'chi' && tx.category !== 'Giá vốn mua hàng' && !tx.refId).slice(0, 3).map((tx) => (
                <div key={tx.id} className="flex items-center justify-between text-xs py-2 border-b border-gray-50">
                  <div className="space-y-0.5">
                    <p className="font-bold text-gray-800">{tx.notes || tx.category}</p>
