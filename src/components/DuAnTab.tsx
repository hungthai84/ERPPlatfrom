  ...
  return (
    ...
    >
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 mb-2">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-2 tracking-tight">Dự án & Công việc</h1>
        </div>
      ...
      {/* Tabs */}
      <div className="flex border-b border-gray-200 dark:border-slate-800 bg-gray-50 dark:bg-slate-900/50 rounded-lg p-1">
        <motion.button whileTap={{ scale: 0.95 }} 
          onClick={() => setActiveTab('projects')}
          className={`flex-1 flex items-center justify-center gap-2 px-4 py-2 rounded-md text-sm font-semibold transition-all ${
            activeTab === 'projects' ? 'bg-white dark:bg-slate-900 text-blue-600 shadow-sm' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:text-gray-300 hover:bg-white dark:bg-slate-900/50'
          }`}
        >
        ...
        <motion.button whileTap={{ scale: 0.95 }} 
          ...
          className={`flex-1 flex items-center justify-center gap-2 px-4 py-2 rounded-md text-sm font-semibold transition-all ${
            activeTab === 'tasks' ? 'bg-white dark:bg-slate-900 text-blue-600 shadow-sm' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:text-gray-300 hover:bg-white dark:bg-slate-900/50'
          }`}
        >
      ...
      {activeTab === 'projects' ? (
        <div className="space-y-6">
          {/* Chart Section */}
          <div className="bg-white dark:bg-slate-900/80 backdrop-blur-md p-6 rounded-2xl border border-gray-100 dark:border-slate-800/50 shadow-sm flex flex-col sm:flex-row gap-6 items-center justify-between">
          ...
          <div className="bg-white dark:bg-slate-900/80 backdrop-blur-md p-6 rounded-2xl border border-gray-100 dark:border-slate-800/50 shadow-sm space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                <h2 className="text-lg font-bold text-slate-800">Danh sách dự án</h2>
                <div className="bg-gray-100 dark:bg-slate-800 p-1 rounded-xl flex items-center gap-1 border border-gray-200 dark:border-slate-800 shadow-inner">
                  <motion.button whileTap={{ scale: 0.95 }} 
              ...
              <div className="flex items-center gap-2">
                <motion.button whileTap={{ scale: 0.95 }}  onClick={handleExportPDF} className="bg-red-50 text-red-600 border border-red-100 px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest hover:bg-red-100 transition-all flex items-center gap-1.5">
                  <Download className="w-3.5 h-3.5" /> PDF
                </motion.button>
                <motion.button whileTap={{ scale: 0.95 }}  onClick={handleExportCSV} className="bg-emerald-50 text-emerald-600 border border-emerald-100 px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest hover:bg-emerald-100 transition-all flex items-center gap-1.5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="relative md:col-span-1">
                ...
                <input
                  ...
                  className="w-full bg-gray-50 dark:bg-slate-900/50 border border-gray-200 dark:border-slate-800 rounded-xl py-2 px-10 text-[11px] font-bold focus:outline-none focus:ring-4 focus:ring-blue-500/10 transition-all"
                />
              </div>
              <div className="flex items-center gap-3">
                ...
                <select
                  ...
                  className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-xl py-2 px-3 text-[11px] font-bold focus:outline-none focus:ring-4 focus:ring-blue-500/10 cursor-pointer"
                >
            ...
            {viewMode === 'list' ? (
              <div className="overflow-x-auto rounded-xl border border-gray-100 dark:border-slate-800/50">
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-gray-50 dark:bg-slate-900/50 border-b border-gray-100 dark:border-slate-800/50">
                      ...
                      <tr key={project.id} className="hover:bg-slate-50/50 transition-all">
                        ...
                        <td className="p-4 text-right">
                          <div className="flex items-center justify-end gap-1">
                            <motion.button whileTap={{ scale: 0.95 }}  onClick={() => openEditModal(project)} className="p-1.5 hover:bg-blue-50 text-blue-600 rounded-lg transition-all"><Edit3 className="w-3.5 h-3.5" /></motion.button>
                            <motion.button whileTap={{ scale: 0.95 }}  onClick={() => handleDeleteProject(project.id)} className="p-1.5 hover:bg-red-50 text-red-600 rounded-lg transition-all"><Trash2 className="w-3.5 h-3.5" /></motion.button>
                          </div>
                        </td>
                      </tr>
                    ))}
              ...
              <div className="h-[200px] bg-gray-50 dark:bg-slate-900/50 rounded-2xl flex items-center justify-center border border-dashed border-gray-200 dark:border-slate-800">
        ...
        <div className="bg-white dark:bg-slate-900/80 backdrop-blur-md p-6 rounded-2xl border border-gray-100 dark:border-slate-800/50 shadow-sm">
           <div className="flex items-center gap-4 mb-6">
            <h3 className="text-xl font-bold text-gray-900 dark:text-white mr-auto">Công việc</h3>
            <motion.button whileTap={{ scale: 0.95 }}  className="p-2 bg-gray-50 dark:bg-slate-900/50 hover:bg-gray-100 dark:bg-slate-800 rounded-lg transition text-gray-600 dark:text-gray-400">
          ...
          <ul className="space-y-3">
            {[
              ...
              <li key={idx} className={`flex items-center justify-between p-4 rounded-xl bg-gray-50 dark:bg-slate-900/50/80 border-l-[6px] shadow-sm hover:shadow-md transition cursor-pointer ${todo.completed ? 'border-blue-500' : 'border-orange-400'}`}>
                <p className={`text-sm font-semibold ${todo.completed ? 'text-gray-500 dark:text-gray-400 line-through' : 'text-gray-900 dark:text-white'}`}>
                  {todo.text}
                </p>
                <motion.button whileTap={{ scale: 0.95 }}  className="p-1.5 hover:bg-gray-200 rounded-md transition">
      ...
      {showAddModal && (
        ...
        >
          <motion.div 
            ...
            className="bg-white dark:bg-slate-900 dark:bg-slate-900 rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden border border-gray-100 dark:border-slate-800/50 dark:border-slate-800"
          >
            <div className="p-5 border-b border-gray-100 dark:border-slate-800/50 flex items-center justify-between bg-gray-50 dark:bg-slate-900/50/50">
              <h3 className="text-lg font-bold text-slate-800 tracking-tight">
                {modalMode === 'add' ? 'Thêm dự án mới' : 'Chỉnh sửa dự án'}
              </h3>
              <motion.button whileTap={{ scale: 0.95 }}  onClick={() => setShowAddModal(false)} className="p-2 hover:bg-gray-200 rounded-lg transition-all">
            ...
            <form onSubmit={handleSaveProject} className="p-6 flex flex-col gap-5">
              <div className="flex flex-col gap-2">
                ...
                <input 
                  ...
                  className="w-full bg-gray-50 dark:bg-slate-900/50 border border-gray-200 dark:border-slate-800 rounded-lg py-2.5 px-3 text-sm font-bold text-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-500/20 transition-all" 
                />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div className="flex flex-col gap-2">
                  ...
                  <select 
                    ...
                    className="w-full bg-gray-50 dark:bg-slate-900/50 border border-gray-200 dark:border-slate-800 rounded-lg py-2.5 px-3 text-sm font-bold text-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-500/20 transition-all cursor-pointer"
                  >
                    ...
                    <motion.button whileTap={{ scale: 0.95 }} 
                      ...
                      className={`flex-1 py-2 rounded-lg text-[10px] font-bold uppercase tracking-widest transition-all border ${
                        currentProject.status === s 
                          ...
                          : 'bg-white dark:bg-slate-900 text-slate-500 border-gray-200 dark:border-slate-800 hover:border-blue-200'
                      }`}
                    >
              ...
              <div className="pt-4">
                <motion.button whileTap={{ scale: 0.95 }}  type="submit" className="w-full bg-blue-600 text-white font-bold py-3 rounded-xl hover:bg-blue-700 transition-all flex items-center justify-center gap-2">
