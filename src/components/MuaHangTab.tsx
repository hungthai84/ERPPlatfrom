  ...
  return (
    <div className="p-6 space-y-6 font-sans bg-gray-50/50 min-h-[calc(100vh-4rem)]">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-gray-900 tracking-tight">Mua hàng & Nhập kho</h1>
          <p className="text-xs text-gray-500 mt-0.5">Quản lý hóa đơn mua gỗ thô, vật tư phụ kiện và công nợ nhà cung cấp</p>
        </div>
      ...
      <div className="bg-white p-4 rounded-xl border border-gray-100 shadow-xs flex items-center justify-between">
        <div className="w-full md:w-80 relative">
          ...
          <input
            ...
            className="w-full bg-gray-50 border border-gray-200 rounded-lg pl-9 pr-4 py-2 text-xs focus:bg-white focus:ring-1 focus:ring-amber-600 focus:border-amber-600 transition outline-none"
          />
        </div>
            ...
            <tbody className="divide-y divide-gray-100 text-xs">
              {filteredPurchases.map((inv) => (
                <tr key={inv.id} className="hover:bg-gray-50/50 transition">
                  ...
                  <td className="py-3 px-4 font-semibold text-gray-800">{inv.supplierName}</td>
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-xs flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-2xl max-w-4xl w-full max-h-[90vh] flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-150">
            ...
            <div className="p-4 bg-amber-600 text-white flex items-center justify-between shrink-0">
              ...
              <button onClick={() => setShowPurchaseModal(false)} className="hover:bg-white/20 p-1 rounded text-white transition">
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <form onSubmit={handleCreatePurchase} className="flex-1 overflow-y-auto p-6 space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-[11px] font-bold text-gray-500 mb-1">Nhà cung cấp vật tư *</label>
                  <select
                    value={selectedSupplierId}
                    onChange={(e) => setSelectedSupplierId(e.target.value)}
                    className="w-full bg-gray-50 border border-gray-200 rounded-lg px-3 py-2 text-xs focus:bg-white focus:ring-1 focus:ring-amber-600 outline-none"
                    required
                  >
                ...
                <div>
                  <label className="block text-[11px] font-bold text-gray-500 mb-1">Ngày lập đơn mua *</label>
                  <input
                    type="date"
                    value={purchaseDate}
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between border-b pb-2">
                  <span className="text-xs font-bold text-gray-800 uppercase tracking-wide">Danh sách vật liệu thô nhập vào</span>
                  <button
                    type="button"
                    onClick={handleAddItemRow}
                    className="flex items-center gap-1 px-3 py-1 bg-gray-50 border border-gray-200 rounded-lg hover:bg-gray-100 text-[11px] font-bold text-amber-700 transition"
                  >
                    ...
                    <div key={index} className="flex flex-col md:flex-row gap-3 items-end md:items-center bg-gray-50 p-3 rounded-lg border border-gray-100 relative group">
                      ...
                      <div className="flex-1 min-w-[220px]">
                        ...
                        <select
                          ...
                          className="w-full bg-white border border-gray-200 rounded-md px-2.5 py-1.5 text-xs focus:ring-1 focus:ring-amber-600 outline-none"
                        >
                          {products.map(p => (
                      ...
                      <div className="w-24">
                        ...
                        <input
                          ...
                          className="w-full bg-white border border-gray-200 rounded-md px-2.5 py-1 text-xs focus:ring-1 focus:ring-amber-600 outline-none text-center"
                        />
                      </div>
                      ...
                      <div className="w-32">
                        ...
                        <input
                          ...
                          className="w-full bg-white border border-gray-200 rounded-md px-2.5 py-1 text-xs focus:ring-1 focus:ring-amber-600 outline-none text-right"
                        />
                      </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2">
                <div className="space-y-4">
                  ...
                  <div className="bg-gray-50/50 p-4 rounded-xl border border-gray-100 space-y-3">
                    ...
                    {isPaid && (
                      <div className="pt-2 animate-in fade-in duration-150">
                        <label className="block text-[10px] font-bold text-gray-500 mb-1">Tài khoản chi tiền</label>
                        <select
                          value={formFundAccountId}
                          onChange={(e) => setFormFundAccountId(e.target.value)}
                          className="w-full bg-white border border-gray-200 rounded-lg px-3 py-1.5 text-xs focus:ring-1 focus:ring-amber-600 outline-none"
                        >
                          {fundAccounts.map(fa => (
                  ...
                  {/* Notes */}
                  <div>
                    <label className="block text-[11px] font-bold text-gray-500 mb-1">Ghi chú giao nhận vật liệu</label>
                    <textarea
                      placeholder="Ghi chú thêm về chuyến gỗ hoặc chứng từ đính kèm..."
                      rows={2.5}
                      value={formNotes}
                      onChange={(e) => setFormNotes(e.target.value)}
                      className="w-full bg-gray-50 border border-gray-200 rounded-lg px-3 py-2 text-xs focus:bg-white focus:ring-1 focus:ring-amber-600 outline-none resize-none"
                    />
                  </div>
                </div>
                ...
                <div className="bg-gray-50 p-4 rounded-xl border border-gray-200 text-xs space-y-3 font-medium">
                  <span className="text-xs font-bold text-gray-800 uppercase tracking-wide block border-b pb-2">Tóm tắt đơn mua</span>
                  
                  <div className="flex justify-between items-center py-1">
                    <span className="text-gray-500">Tổng tiền hàng nhập xưởng:</span>
                    <span className="text-gray-900 font-bold">{formatVND(formSubtotal)}</span>
                  </div>

                  <div className="flex justify-between items-center py-1">
                    <span className="text-gray-500">Giảm giá của nhà cung cấp (đ):</span>
                    <input
                      type="number"
                      value={discount}
                  ...
                  <div className="border-t pt-2.5 flex justify-between items-center text-sm font-extrabold text-gray-900 bg-white/40 p-2.5 rounded-lg border">
                    <span className="text-gray-800">Tổng tiền phải trả:</span>
